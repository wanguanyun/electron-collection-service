...
router.get('/top', (req, res, next) => {
  db.query(`SELECT * FROM (SELECT * FROM (SELECT a.id,a.title,a.author,a.tag,a.release_date,a.status,
      ...
      GROUP_CONCAT(DISTINCT(b.name)) AS category FROM blog_article AS a 
      LEFT JOIN (SELECT aa.*,bb.name FROM blog_article_category AS aa LEFT JOIN blog_category AS bb ON aa.category_id = bb.id) AS b ON a.id = b.article_id
      WHERE a.status = 2 and a.if_front = 1 GROUP BY a.id) AS c ORDER BY c.release_date DESC LIMIT 0,4) AS d LEFT JOIN (SELECT id AS img_id,name,file_type,net_url FROM blog_img) 
...
router.post('/random', (req, res, next) => {
  const param = req.body;
  let cate_sql = param.categoryId ? `WHERE FIND_IN_SET('${param.categoryId}',c.category_id)`:''
  db.query(`SELECT * FROM (SELECT * FROM (SELECT a.id,a.title,a.author,a.tag,a.release_date,a.status,
      a.if_front,a.cover_img,a.summary,a.if_allow_comment,GROUP_CONCAT(DISTINCT(b.category_id)) AS category_id,
      GROUP_CONCAT(DISTINCT(b.name)) AS category FROM blog_article AS a 
      LEFT JOIN (SELECT aa.*,bb.name FROM blog_article_category AS aa LEFT JOIN blog_category AS bb ON aa.category_id = bb.id) AS b ON a.id = b.article_id
      WHERE a.status = 2 GROUP BY a.id) AS c ${cate_sql} ORDER BY c.release_date DESC LIMIT 0,10) AS d LEFT JOIN (SELECT id AS img_id,name,file_type,net_url FROM blog_img) 
      AS e ON d.cover_img = e.img_id`).then((data) => {
    //随机从数组中选取n个
    let randomResult = data.rows.sort(() => 0.5 - Math.random()).slice(0, 10);
